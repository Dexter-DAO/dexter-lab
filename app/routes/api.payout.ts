 ...
 * Payout API Route
 *
 * Proxies manual payout requests to dexter-api.
 * POST /api/payout - Trigger a payout for a lab resource
 */
...
export const action: ActionFunction = async ({ request }) => {
  ...
  try {
    const res = await fetch(`${DEXTER_API_BASE}/api/dexter-lab/payouts/${encodeURIComponent(resourceId)}`, {
      method: 'POST',
      headers: {
    ...
    console.error('[api.payout] Payout request failed:', error);
    return json({ error: 'Payout request failed' }, { status: 500 });
  }
};
