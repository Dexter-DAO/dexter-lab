...
export default memo(
  ({
    ...
    const perfBoost = isMobile ? 1.8 : 1;
    const scaledParticleSize = Math.max(1, particleSize * sizeRatio * perfBoost);
    const scaledParticleGap = Math.max(2, Math.round(particleGap * sizeRatio * perfBoost));
    ...
    const generateParticles = useCallback(() => {
      ...
      // Extra margin so scattered particles aren't clipped at canvas edges
      const scatterMargin = responsiveFontSize * 0.5;
    ...
    const animate = useCallback(() => {
      ...
      particles.forEach((particle) => {
        ...
        // Glow (only after settled -- radial gradients are expensive during intro)
        if (particle.arrived) {
